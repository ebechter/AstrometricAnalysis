...
dprlx   = values[ind][1]   # mas
pm_ra   = values[ind][2]   # mas/yr
dpm_ra  = values[ind][3]   # mas/yr
pm_dec  = values[ind][4]   # mas/yr
dpm_dec = values[ind][5]   # mas/yr
# print(prlx)
# 1/0
...
tl2 = np.linspace(np.min(t), np.ceil(np.max(t)), npoints)
...
fig.savefig(target + '_plot_all.eps')
...
axarr[0].set_ylabel("$\Delta$ North (mas)",fontweight='normal',fontsize=16,labelpad = 4,**csfont)
...
axarr[1].fill_between(tl, EW_vector-sigma_ew,EW_vector+sigma_ew,alpha=0.2,color='black',linewidth=0)
...
axarr[1].set_ylabel("$\Delta$ East (mas)",fontweight='normal',fontsize=16,labelpad = -2,**csfont)
axarr[1].set_xlabel("Date (years)",fontweight='normal',fontsize=16,labelpad = -2,**csfont)
...
pl.savefig(target + '_plot_NSEW.pdf')
